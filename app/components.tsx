...
export function Header() {
  ...
  return (
    <header className="academy-header">
      <div className="academy-header-shell">
        <Link className="academy-wordmark" href="/" aria-label="Interprete., voltar ao início">
          <BrandMark />
          <span>Interprete<span className="academy-wordmark-dot">.</span></span>
        </Link>
        <nav className="academy-desktop-nav" aria-label="Navegação principal">
        ...
        <div className="academy-header-actions">
          <Link className="academy-header-secondary" href="/planos">Ver formatos</Link>
          <a className="academy-header-primary" href={contactHref} target="_blank" rel="noreferrer">
    ...
    <footer className="academy-footer">
      <div className="academy-container academy-footer-grid">
        <div className="academy-footer-brand">
          ...
          <p>Leitura crítica e aplicação de evidências para decisões mais conscientes.</p>
        </div>
        <nav className="academy-footer-column" aria-label="Plataforma">
        ...
        <nav className="academy-footer-column" aria-label="Institucional">
        ...
        <div className="academy-footer-next">
      ...
      <div className="academy-container academy-footer-bottom">
        <small>© {new Date().getFullYear()} Interprete. Todos os direitos reservados.</small>
      </div>
      <a className="academy-whatsapp academy-whatsapp-pulse" href={contactUrl()} target="_blank" rel="noreferrer" aria-label="Conversar com o Interprete. pelo WhatsApp">
...
export function Button({
  ...
  const classes = ['academy-button', secondary ? 'academy-button--secondary' : '', className].filter(Boolean).join(' ');
...
export function SectionLabel({ children }: { children: React.ReactNode }) {
  return <p className="academy-eyebrow">{children}</p>;
}

export function PageShell({ children }: { children: React.ReactNode }) {
  return <div data-theme="interprete-academy">{children}</div>;
}
